...
class Specimen(object):
    ...
    """
    interface.implements(ISpecimen)
    ...
    def __init__(self, **kwargs):
        ...
        if self.destination is None:
            self.destination = u"Richman Lab"
        if self.state is None:
            self.state = u'pending-draw'

    @property
    def protocol_title(self):
        return utils.get_protocol_title(self.protocol_zid)
    @property
    def study_title(self):
        return utils.get_study_title(self.protocol_zid)

    @property
    def patient_title(self):
        return utils.get_patient_title(self.subject_zid)

    @property
    def patient_initials(self):
        return utils.get_patient_initials(self.subject_zid)

    @property
    def patient_legacy_number(self):
        return utils.get_patient_legacy_number(self.subject_zid)
